...
$sql = "SELECT COUNT(*) AS count, SUM(distance) AS distance, SUM(TIME_TO_SEC(duration))/60 AS duration FROM flights where uid=1" . $filter;
...
$sql = "SELECT DISTINCT s.apid,s.x,s.y,d.apid,d.x,d.y,count(fid) AS times FROM flights AS f, airports AS s, airports AS d WHERE f.uid=1 AND f.src_apid=s.apid AND f.dst_apid=d.apid " . $filter . " GROUP BY fid";
$result = mysql_query($sql, $db);
$first = true;
...
$sql = "SELECT DISTINCT a.apid,x,y,name,iata,icao,city,country,count(name) AS visits FROM flights AS f, airports AS a WHERE f.uid=1 AND (f.src_apid=a.apid OR f.dst_apid=a.apid) " . $filter . " GROUP BY name";
$result = mysql_query($sql, $db);
$first = true;
